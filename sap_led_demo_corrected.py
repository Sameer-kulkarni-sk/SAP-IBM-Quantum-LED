#!/usr/bin/env python3
"""
LED SAP Demo - Corrected Version
Uses 1D pixel list like IBM demo (not 2D matrix)
Matches IBM Quantum LED demo exactly
"""
...
# 64-element 1D list like IBM demo
pixels = [O[:] for _ in range(TOTAL_PIXELS)]
...
def draw_sap(rainbow=False, hue_offset=0.0):
    ...
    # SAP pattern on 8x8 grid (simplified for 8x8)
    # S pattern (rows 1-2)
    s_pixels = [9, 10, 11, 16, 24, 25, 26, 27, 35, 43, 50, 51, 52]
    # A pattern (rows 3-4)
    a_pixels = [18, 19, 20, 25, 27, 33, 35, 41, 42, 43, 44, 45, 49, 51, 57, 59]
    # P pattern (rows 5-6)
    p_pixels = [37, 38, 39, 40, 45, 47, 53, 55, 61, 62, 63, 64]

    # Draw S
    for idx in s_pixels:
        if idx < TOTAL_PIXELS:
            if rainbow:
                pixels[idx] = get_rainbow_color(idx // 8, 8, hue_offset)
            else:
                pixels[idx] = COLOR_SAP_BLUE[:]
...
def main():
    """Main program"""
    print("\n" + "="*50)
    print("SAP LED Demo - Corrected Version")
    print("Using 1D pixel list like IBM demo")
    print("="*50 + "\n")
